	...
	internal class DropDownListSource: UITableViewSource
	{
		...
		public override UITableViewCell GetCell (UITableView tableView, MonoTouch.Foundation.NSIndexPath indexPath)
		{
			...
			if (item.Image != null) {
				...
				var newSize = new SizeF (25f, 25f);
				if(m_ImageSize>0.0f)
					newSize = new SizeF (m_ImageSize, m_ImageSize);
				UIGraphics.BeginImageContextWithOptions (newSize, false, UIScreen.MainScreen.Scale);
				cell.ImageView.Image.Draw (new RectangleF (0, 0, newSize.Width, newSize.Height));
